...
namespace WFA_EJ.Forms
{
    public partial class F_Journal : Form
    {
        ...
        public F_Journal(DateTime Date, (Group group, Teacher teacher, Subject subject) selected)
        {
            ...
            StudentsFromTheSelectedGroup = Program.DataBase.DataBaseEntity.Students
                .Where(x => x.GroupGuid == Selected.group.Guid).ToList();
            ...
            EvalutionsInput = Program.DataBase.DataBaseEntity.EvaluationOfStudents.Where(x =>
                x.TeacherGuid == selected.teacher.Guid && x.SubjectGuid == selected.subject.Guid &&
                Selected.group.Students.Contains(x.StudentGuid)).ToList();
            foreach (var evaluation_of_student in EvalutionsInput)
            {
                var indexStudent =
                    StudentsFromTheSelectedGroup.IndexOf(
                        Program.DataBase.DataBaseEntity.Students.First(x =>
                            x.Guid == evaluation_of_student.StudentGuid));
                dataGridView1[evaluation_of_student.date_time.Day, indexStudent].Value =
                    status[(int) evaluation_of_student.Evaluation];
            }
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            for (var ColumnIndex = 0; ColumnIndex < _DaysInMonth; ColumnIndex++)
            {
                ...
                {
                    Program.DataBase.DataBaseEntity.EvaluationOfStudents.First(x => x.Guid == EvalutionsInput
                        .First(EvaluationOfStudent =>
                            EvaluationOfStudent.StudentGuid == StudentsFromTheSelectedGroup[RowIndex].Guid
                            && EvaluationOfStudent.date_time.Day == ColumnIndex)
                        .Guid).Evaluation = GetEvaluationEnum(tableOut[RowIndex, ColumnIndex]);
                }
            }
